...
try {
     ...
     * Sorting setup
     */
    $allowedSortFields = ["supplier_name", "supplier_number", "wht_status", "created_at", "id"];
     ...
     * Base query
     */
    $baseQuery = "FROM suppliers_table WHERE 1=1";
    $params = [];
    $types  = "";

    /**
     * Search filter
     * (Search by supplier_name or supplier_number)
     */
    if ($search) {
        $baseQuery .= " AND (supplier_name LIKE ? OR supplier_number LIKE ?)";
        $likeSearch = "%" . $search . "%";
        $params[] = $likeSearch;
        $params[] = $likeSearch;
        $types .= "ss";
