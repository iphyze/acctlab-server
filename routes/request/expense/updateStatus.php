...
try {
    ...
    $checkStmt = $conn->prepare("SELECT id FROM expense_fund_request_table WHERE id IN ($placeholders)");
    ...
    try {
        $updateQuery = "UPDATE expense_fund_request_table SET payment_status = ? WHERE id IN ($placeholders)";
        ...
        $logAction = "$loggedInUserEmail updated payment_status to '$paymentStatus' for request(s) with ID(s): " . implode(', ', $requestIds) . " in Expenses Payment Request.";
