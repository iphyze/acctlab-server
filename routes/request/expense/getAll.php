...
try {
    ...
    if ($payment_status === null || $payment_status === '' || $payment_status === 'All') {
        $statement = "SELECT * FROM expense_fund_request_table WHERE YEAR(created_at) = ? ORDER BY created_at DESC";
        ...
        $statement = "SELECT * FROM expense_fund_request_table WHERE payment_status = ? AND YEAR(created_at) = ? ORDER BY created_at DESC";
        $get = $conn->prepare($statement);
        if (!$get) {
            throw new Exception("Failed to prepare statement: " . $conn->error, 500);
        }
