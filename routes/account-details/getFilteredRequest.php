...
try {
     ...
     * Sorting setup
     */
    $allowedSortFields = ["account_name", "account_number", "bank_name", "created_at", "id"];
     ...
     * Base query
     */
    $baseQuery = "FROM suppliers_account_details WHERE 1=1";
    $params = [];
    $types  = "";

    /**
     * Search filter
     * (Search by account_name, account_number, or bank_name)
     */
    if ($search) {
        $baseQuery .= " AND (account_name LIKE ? OR account_number LIKE ? OR bank_name LIKE ?)";
        $likeSearch = "%" . $search . "%";
        $params[] = $likeSearch;
        $params[] = $likeSearch;
        $params[] = $likeSearch;
    ...
    echo json_encode([
        "status" => "Success",
        "message" => "Supplier account details fetched successfully",
        "data" => $data,
        "meta" => [
            "total" => (int) $total,
            "limit" => $limit,
