...
try {
    ...
    if (!isset($data['scheduleIds']) || !is_array($data['scheduleIds']) || count($data['scheduleIds']) === 0) {
        throw new Exception("Please select a payment first.", 400);
    }

    $scheduleIds = array_map('intval', $data['scheduleIds']);

    // Start transaction
    $conn->begin_transaction();

    try {
        // Delete from union_payment_schedule table
        $placeholders = implode(',', array_fill(0, count($scheduleIds), '?'));
        $deleteQuery = "DELETE FROM union_payment_schedule WHERE id IN ($placeholders)";
        ...
        $deleteStmt->bind_param(str_repeat('i', count($scheduleIds)), ...$scheduleIds);
        ...
        $logAction = "$loggedInUserEmail deleted payment(s) with ID(s): " . implode(', ', $scheduleIds);
